...
$wmClusterInput = @{
    ...
    masterDnsSearchDomain = "cmi.elasticsky.org"
   }
...
certreq -submit -config $CAName -attrib "CertificateTemplate:$Template" $csrFile

# Install the newly created certificate
logger " Installing Tanzu certificate"
Install-SupervisorClusterCertificate -server $sddcManagerFqdn -user $sddcManagerUser -pass $sddcManagerPass -domain $sddcDomainName -Cluster $wmClusterName -filePath "./SuperCert.cer" -
...
$wmNamespaceEditUserGroup = "gg-kub-admins"
$wmNamespaceViewUserGroup = "gg-kub-readonly"
Add-NamespacePermission -server $sddcManagerFqdn -user $sddcManagerUser -pass $sddcManagerPass -sddcDomain $sddcDomainName -domain $domainFqdn -domainBindUser $domainBindUser -domainBindPass $domainBindPass -namespace $wmTkcNamespaceName -principal $wmNamespaceEditUserGroup -role edit -type group -disconnect false
Add-NamespacePermission -server $sddcManagerFqdn -user $sddcManagerUser -pass $sddcManagerPass -sddcDomain $sddcDomainName -domain $domainFqdn -domainBindUser $domainBindUser -domainBindPass $domainBindPass -namespace $wmTkcNamespaceName -principal $wmNamespaceViewUserGroup -role view -type group -disconnect false
